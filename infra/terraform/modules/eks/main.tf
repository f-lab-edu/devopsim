...
resource "aws_eks_cluster" "this" {
  ...
  vpc_config {
    ...
    endpoint_public_access  = true # kubectl 로컬 접근용
  }
...
# OIDC Provider — IRSA(Pod에 IAM Role 부여)의 기반
...
# EBS CSI Driver Add-on — StatefulSet PVC 지원
...
# ALB Controller IAM Role (IRSA) — Helm 설치는 week5
...
# External Secrets IAM Role (IRSA) — Helm 설치는 week5
